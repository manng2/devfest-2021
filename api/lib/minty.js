...
const convertImage = require('base64-arraybuffer')
...
class Minty {
    ...
    async getIPFS(cidOrURI) {
        ...
        const x = await all(this.ipfs.cat(cid))

        return uint8ArrayConcat.concat(await all(this.ipfs.cat(cid)))
    }
    ...
    async getIPFSString(cidOrURI) {
        ...
        return uint8ArrayToString.toString(bytes)
    }
    ...
    async getIPFSBase64(cidOrURI) {
        const bytes = await this.getIPFS(cidOrURI)
        return uint8ArrayToString.toString(bytes, 'base64')
    }
